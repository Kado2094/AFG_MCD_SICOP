...
- Loads every CSV under one or more GCS prefixes (months) into BigQuery tables.
...
# You can load one or many month-folders (prefixes).
# Example: ["sicop-csv/202511/"]
# Future: ["sicop-csv/202501/", "sicop-csv/202502/", ...]
PREFIXES = [
    "sicop-csv/202112/",
    "sicop-csv/202111/",
    "sicop-csv/202110/",
...
# If you want tables per month (recommended once you load many months):
# - False: tables are append each month (same table name), verify WRITE_DISPOSITION
# - True: creates tables like LINEASCONTRATADAS_202511, etc.
...
# Optional fallback: disable quote processing
...
def extract_month_suffix_from_prefix(prefix: str) -> Optional[str]:
    """
    Expecting something like 'sicop-csv/202511/' -> returns '202511'
    If not in that format, returns None.
    """
    parts = prefix.strip("/").split("/")
    if not parts:
        return None
    last = parts[-1]
    return last if re.fullmatch(r"\d{6}", last) else None
...
def load_csv_to_bq(
    ...
    blob_name: str,
    month_suffix: Optional[str]
) -> LoadOutcome:
    filename = blob_name.split("/")[-1]
    base_table_name = sanitize_bq_table_name(filename.replace(".csv", ""))

    # Optional monthly suffix to avoid overwriting when you start loading many months
    if SUFFIX_TABLE_WITH_MONTH and month_suffix:
        table_name = f"{base_table_name}_{month_suffix}"
    else:
    ...
    try:
        ...
        try:
            ...
            return LoadOutcome(uri=uri, table_id=table_id, ok=True, job_id=job.job_id, output_rows=job.output_rows)
...
def main() -> None:
    ...
    for prefix in PREFIXES:
        month_suffix = extract_month_suffix_from_prefix(prefix)
        logger.info("=== Processing prefix: %s | month_suffix=%s ===", prefix, month_suffix)
        ...
        for blob_name in csv_blobs:
            outcomes.append(load_csv_to_bq(bq_client, storage_client, logger, blob_name, month_suffix))
    ...
    # Quantitative traceability recap
    if ok:
