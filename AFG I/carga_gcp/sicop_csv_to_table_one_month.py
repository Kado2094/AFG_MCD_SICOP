#!/usr/bin/env python3
"""
SICOP Bronze Loader: GCS -> BigQuery
- Loads every CSV under a GCS prefix into its own BigQuery table.
- Forces ALL columns to STRING (stable bronze layer).
...
PREFIX = "sicop-csv/202511/"  # folder with all CSVs for the month

# Default delimiter preference (per-file autodetect may override)
DEFAULT_DELIMITER = ";"
CSV_ENCODING = "utf-8"

# Overwrite tables each run (bronze refresh). EXTRA OPTIONS ARE CHANGE BY CHANGING WRITE_TRUNCATE
...
# If True: allow some bad rows (skip them) to complete the load.
ENABLE_TOLERANT_MODE = True
MAX_BAD_RECORDS_TOLERANT = 500  # adjust as needed

# If True: second attempt with quote_character disabled for tricky quote issues.
ENABLE_QUOTE_FALLBACK = True

# --- Logging to file ---
ENABLE_FILE_LOGGING = True  # turn on/off adjust as needed
LOG_DIR = "logs"            # created locally
LOG_LEVEL = "INFO"

HEADER_MAX_BYTES = 256 * 1024  # bytes to read from start to capture header line
...
def init_logger() -> logging.Logger:
    logger = logging.getLogger("sicop_bronze_loader")
    ...
    if ENABLE_FILE_LOGGING:
        ...
        log_path = os.path.join(LOG_DIR, f"sicop_bq_load_{ts}.txt")
...
def detect_delimiter_from_header_line(header_line: str, default_delim: str = ";") -> str:
    """
    Detect delimiter based on which candidate yields more columns.
    Supports ';' and ',' for your current dataset realities.
    """
...
def read_header_from_gcs(
    ...
    """
    Reads header and returns (columns, detected_delimiter).
    """
    header_line = read_first_nonempty_line(storage_client, bucket_name, blob_name, encoding, max_bytes)
...
def run_bq_load_job(
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        quote_character=quote_character,  # can be None
    ...
    job = bq_client.load_table_from_uri(uri, table_id, location=BQ_LOCATION, job_config=job_config, retry=retry)
    job.result()
...
def load_csv_to_bq(
    ...
    table_name = sanitize_bq_table_name(filename.replace(".csv", ""))
    ...
    try:
        ...
        # Strict first attempt (best quality)
        max_bad = 0
        if ENABLE_TOLERANT_MODE:
            max_bad = MAX_BAD_RECORDS_TOLERANT

        # Attempt 1: normal quoting
        try:
            run_bq_load_job(
                bq_client=bq_client,
                uri=uri,
            ...
            logger.info("OK: %s", table_id)
            return LoadOutcome(uri=uri, table_id=table_id, ok=True)

        except BadRequest as e1:
            # Optional fallback: disable quote processing to survive malformed quote patterns
            if ENABLE_QUOTE_FALLBACK:
                ...
                logger.info("OK (fallback): %s", table_id)
                return LoadOutcome(uri=uri, table_id=table_id, ok=True)
...
def main() -> None:
    ...
    logger.info("Source=gs://%s/%s", BUCKET_NAME, PREFIX)

    bq_client = bigquery.Client(project=PROJECT_ID, location=BQ_LOCATION)
    storage_client = storage.Client(project=PROJECT_ID)

    csv_blobs = list_csv_blobs(storage_client, BUCKET_NAME, PREFIX)
    logger.info("Found %d CSV files.", len(csv_blobs))
